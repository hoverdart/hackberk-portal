...
const criteria = parseRubric({ version: 1, criteria: [{ key: "motivation", label: "Motivation", weight: 0.6 }, { key: "community", label: "Community", weight: 0.4 }] });

describe("blind-review scoring", () => {
  it("ignores drafts when calculating the organizer aggregate", () => {
    expect(aggregateSubmittedReviews([{ status: "draft", scores: { motivation: 5, community: 5 } }], criteria)).toBeNull();
  });

  it("applies rubric weights across submitted independent reviews", () => {
    expect(aggregateSubmittedReviews([
