...
export type ApplicationStatus = "draft" | "submitted" | "under_review" | "accepted" | "waitlisted" | "rejected" | "withdrawn";
...
type EventRow = { id: string; slug: string; name: string; tagline: string; venue: string; timezone: string; starts_at: string; ends_at: string; applications_open_at: string; applications_close_at: string; application_forms: Json; application_rubric: Json; project_rubric: Json; is_active: boolean; is_synthetic: boolean; created_at: string; updated_at: string };
type ProfileRow = { id: string; full_name: string; preferred_name: string | null; school: string | null; graduation_year: number | null; pronouns: string | null; timezone: string; created_at: string; updated_at: string };
type ApplicationRow = { id: string; event_id: string; applicant_id: string; role: ApplicationRole; status: ApplicationStatus; form_version: number; lock_version: number; submitted_at: string | null; withdrawn_at: string | null; decided_at: string | null; created_at: string; updated_at: string };
type AnswerRow = { id: string; application_id: string; section_key: string; answers: Json; is_identity_sensitive: boolean; answer_version: number; updated_at: string };
type GenericRow = Record<string, any>;

export type Database = {
  public: {
    Tables: {
      events: Table<EventRow>; profiles: Table<ProfileRow>; staff_members: Table<GenericRow>;
      applications: Table<ApplicationRow>; application_answers: Table<AnswerRow>; review_assignments: Table<GenericRow>;
      application_reviews: Table<GenericRow>; audit_log: Table<GenericRow>; matching_profiles: Table<GenericRow>;
      teams: Table<GenericRow>; team_members: Table<GenericRow>; team_invitations: Table<GenericRow>;
      projects: Table<GenericRow>; project_review_assignments: Table<GenericRow>; project_reviews: Table<GenericRow>;
