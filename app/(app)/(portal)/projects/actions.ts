...
const projectSchema = z.object({
  ...
  demoUrl: z
    .string()
    .trim()
    .refine((value) => !value || /^https?:\/\//.test(value), "Use a full URL."),
});
...
export async function saveProjectAction(
  ...
  const payload = {
    ...
    github_url: parsed.data.githubUrl.canonicalUrl,
...
export async function refreshProjectMetadataAction(projectIdValue: string) {
  ...
  try {
    ...
    await supabase
      .from("projects")
      .update({ github_cache: snapshot as unknown as Json, github_cached_at: new Date().toISOString() })
      .eq("id", projectId);
  } catch {
