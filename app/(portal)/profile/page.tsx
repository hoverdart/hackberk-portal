...
/** A paper account sheet with the user's profile details and separate role records. */
export default async function ProfilePage() {
  ...
  const { data: applicationRows } = event
  ...
  const name = profile?.preferred_name || profile?.full_name || "Applicant";
  const details = [
    ...
    <main className="profile-page">
      <section className="profile-sheet">
        ...
        <section className="profile-roles" aria-labelledby="profile-roles-title">
          <h2 id="profile-roles-title">Your roles</h2>
          <p>Each role has its own application and status.</p>
          <ul>
