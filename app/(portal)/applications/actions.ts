...
export async function startApplicationAction(eventId: string, roleValue: string) {
  ...
  const { error } = await supabase
    .from("applications")
    .upsert(
      { event_id: event, applicant_id: user.id, role, status: "draft" },
      { onConflict: "event_id,applicant_id,role", ignoreDuplicates: true },
    );
...
export async function saveApplicationSectionAction(
  ...
  if (!parsed.success) {
    const errors = Object.fromEntries(
      Object.entries(parsed.error.flatten().fieldErrors).filter((entry): entry is [string, string[]] =>
        Boolean(entry[1]),
      ),
    );
  ...
  const { data: application } = await supabase
    .from("applications")
    .select("status,applicant_id,role")
    .eq("id", applicationId)
    .eq("applicant_id", user.id)
    .maybeSingle();
  if (!application || application.role !== role)
    return { status: "error", message: "Application not found. Refresh this page and try again." };
  ...
  if (application.status !== "draft")
    return {
      status: "error",
      message: "Submitted answers are locked. Withdraw the application before making changes.",
    };
  ...
  if (expectedVersion > 0) {
    const { data, error } = await supabase
      ...
      .update({ answers: parsed.data as Json, is_identity_sensitive: section.identitySensitive })
      .eq("application_id", applicationId)
      .eq("section_key", sectionKey)
      .eq("answer_version", expectedVersion)
      .select("answer_version")
      .maybeSingle();
    if (error) return { status: "error", message: "Draft save failed. Your answers remain on this page." };
    if (!data)
      return {
        status: "stale",
        message: "This draft changed elsewhere. Refresh before saving so you do not overwrite newer answers.",
      };
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      answers: parsed.data as Json,
      is_identity_sensitive: section.identitySensitive,
    })
    .select("answer_version")
    .single();
  ...
  if (error?.code === "23505")
    return { status: "stale", message: "This section was created in another tab. Refresh before saving." };
...
export async function submitApplicationAction(applicationIdValue: string, roleValue: string) {
  ...
  const { data: application } = await supabase
    .from("applications")
    .select("status,event_id,role")
    .eq("id", applicationId)
    .eq("applicant_id", user.id)
    .maybeSingle();
  if (!application || application.role !== role || application.status !== "draft")
    redirect(`/applications/${role}?error=not-draft`);
  const { data: answerRows } = await supabase
    .from("application_answers")
    .select("section_key,answers")
    .eq("application_id", applicationId);
  const answers = Object.fromEntries((answerRows ?? []).map((row) => [row.section_key, row.answers]));
  const invalidSection = getApplicationDefinition(role).find(
    (section) => !schemaForSection(section).safeParse(answers[section.key] ?? {}).success,
  );
...
export async function withdrawApplicationAction(applicationIdValue: string, roleValue: string) {
  ...
  const { data, error } = await supabase
    ...
    .select("id")
    .maybeSingle();
  if (error || !data) redirect(`/applications/${role}?error=withdraw`);
  revalidatePath("/dashboard");
  revalidatePath(`/applications/${role}`);
